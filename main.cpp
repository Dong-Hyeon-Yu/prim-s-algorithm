...
private:
 ...
 //   }
    ...
    void _downHeap(_Iter root) {
        _Iter smaller;
        if (left_child_idx(root) >= c.size()) return;
        else if (right_child_idx(root) >= c.size()) smaller = left_child(root);
        else smaller = comp(*left_child(root), *right_child(root)) ? left_child(root) : right_child(root);

        if (!comp(*root, *smaller)) {
            iter_swap(root, smaller);
            _downHeap(smaller);
        }
    }
public:
